...
final class RawMimeAttachmentProvider implements AttachmentProviderInterface
{
    public function get(MessageRef $ref): iterable
    {
        ...
        try { $parser->setText($raw); } catch (\Throwable) { return; }
        ...
        try {
            foreach ($parser->getAttachments() as $a) {
                try {
                    ...
                    if (strlen($data) > 25 * 1024 * 1024) continue;
    ...
    {
        if (class_exists(\PhpMimeMailParser\Parser::class)) return new \PhpMimeMailParser\Parser();
        if (class_exists(\eXorus\PhpMimeMailParser\Parser::class)) return new \eXorus\PhpMimeMailParser\Parser();
        return null;
    }
